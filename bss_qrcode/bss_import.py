...
FAIL = "fail"
NOT_FOUND = "not_found"
FILENAME_QRCODE_NOT_FOUND = "qrcode_not_found.pdf"
PROCESSED = "processed"
...
class bss_imported_document(osv.osv):
    ...
    _columns = {
        'import_id': fields.many2one(
            'bss_qrcode.import', string="Imported date", ondelete='cascade',
        ...
        'qrcode_create_date': fields.related(
            'qrcode_id', 'create_date', type='char',
            string='First download date', store=False
        ),
        'oe_version': fields.related(
            'qrcode_id', 'oe_version', type='char', string='Version',
            store=False
        ),
        'oe_object': fields.related(
            'qrcode_id', 'oe_object', type='char', string='Object',
            store=False
        ),
        'oe_id': fields.related(
            'qrcode_id', 'oe_id', type='char', string='Id', store=False
        ),
        'user_id': fields.related(
            'qrcode_id', 'user_id', type='char', string='User id', store=False
        ),
        'report': fields.related(
            'qrcode_id', 'report', type='char', string='Report', store=False
        ),
        'filename': fields.related(
            'qrcode_id', 'filename', type='char', string='Filename',
            store=False
        ),
    }
    ...
    def onchange_state(self, cr, uid, ids, context=None):
        ...
        for imported_document in self.browse(cr, uid, ids, context):
            myimport = self.pool.get('bss_qrcode.import').browse(
                cr, uid, imported_document.import_id.id, context
            )
            count_unprocessed = self.search(
                cr, uid, [('import_id', '=', myimport.id),
                          ('state', '=', UNPROCESSED)], count=True,
                context=context
            )
    ...
    def action_processed(self, cr, uid, ids, context=None):
        ...
        for imported_document in self.browse(cr, uid, ids, context):
            self.write(
                cr, uid, imported_document.id, {'state': PROCESSED}, context
            )
    ...
    def action_unprocessed(self, cr, uid, ids, context=None):
        ...
        for imported_document in self.browse(cr, uid, ids, context):
            self.write(
                cr, uid, imported_document.id, {'state': UNPROCESSED}, context
            )
            self.onchange_state(cr, uid, ids)
        return 1
...
class bss_import(osv.osv):
    ...
    def get_nb(self, cr, uid, ids, name, arg, context=None):
        """ Get the child number (i.e imported documents) which have a
        specific status. """
        res = {}
        bss_imported_document = self.pool.get('bss_qrcode.imported_document')

        for import_id in ids:
            res[import_id] = bss_imported_document.search(
                cr, uid, [('import_id', '=', import_id),
                          ('status', 'like', arg['status'])],
    ...
    _columns = {
        ...
        'imported_document_ids': fields.one2many(
            'bss_qrcode.imported_document', 'import_id',
            string='Imported documents', readonly=True
        ),
        'status': fields.selection(
            [('success', 'All documents succeed'),
             ('fail', 'At least one failed document')
        ...
        'progression': fields.selection(
            [('finished', 'Finished'),
             ...
             ('error', 'Error')
             ], 'Progression', required=True, readonly=True
        ),
        'success_nb': fields.function(
            get_nb, arg={'status': 'success'}, method=True, store=False,
            string="Number of successes", type="integer", readonly=True
        ),
        'fail_nb': fields.function(
            get_nb, arg={'status': 'fail'}, method=True, store=False,
            string="Number of fails", type="integer", readonly=True
        ),
        'not_found_nb': fields.function(
            get_nb, arg={'status': 'not_found'}, method=True, store=False,
            string="Number of not found", type="integer", readonly=True
        ),
        'total': fields.function(
            get_nb, arg={'status': '%'}, method=True, store=False,
            string="Total", type="integer", readonly=True
        ),
    }
    ...
    def process_document(self, cr, uid, myimport, qrcode_id, document):
        """ Process document by creating the imported document and attach the
            file to it.
            Moreover attach the file to the concerned object or make a
            custom action.  """
        qrcode = None
        ...
        if qrcode_id == 0:
            ...
            qrcode = self.pool.get('bss_qrcode.qrcode').read(
                cr, uid, qrcode_id
            )
            ...
            if(qrcode is None):
                ...
                myobject = self.pool.get(qrcode['oe_object']).read(
                    cr, uid, qrcode['oe_id']
                )
                ...
                # 4. The document doesn't have any problem
                    ...
                    # If the qrcode_custom_treatment exists, then we use
                    # the specific treatment
                    try:
                        getattr(
                            self.pool.get(qrcode['oe_object']),
                            "qrcode_custom_treatment"
                        )
                        self.pool.get(qrcode['oe_object']).\
                            qrcode_custom_treatment(cr, uid, qrcode, document)
                    # Else we use the default treatment i.e to attach the
                    # file to the object
                    except AttributeError:
                        self.pool.get('bss_qrcode.qrcode').attach_file(
                            cr, uid, qrcode['id'], document
                        )
    ...
    def add_to_import_object(self, cr, uid, import_id, qrcode_id, document):
        """ Function called by an xmlrpc connection. Create an import object
            with documents. """
